...
void MainWindow::openImage(QImage image, QPixmap mask)
{
 ...
 // connect(stateBox->_scene, SIGNAL(mouseMoved(QPointF, QPointF)), this, SLOT(paintMask(QPointF, QPointF)));
  //(stateBox->_scene)->setBackgroundBrush(palette().dark());
  _imgItem = _scene->addPixmap(QPixmap::fromImage(_img));
  //_imgItem = (stateBox->_scene)->addPixmap(QPixmap::fromImage(_img));
